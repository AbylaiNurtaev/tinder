  ...
  useEffect(() => {
    try {
      ...
      if (userData) {
        ...
        axios.post('/api/validate-init-data', { initData: userObj.id })
          .then(response => {
            console.log(response.data)
            updateFilter('telegramId', response.data.telegramId);
            console.log('filters', filters)
            if (response.data?.user?._id) {
              localStorage.setItem("userId", response.data.user._id);
              setUser(response.data.user); // Устанавливаем userId из ответа
            }
          })
          .catch(error => {
